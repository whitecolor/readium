...
(function($) {
	...
	var calcOffset = function(elem, parent) {
		...
		// subtract the parents offset
		while(parent) {
			offsets.x -= parent.offsetLeft;
			offsets.y -= parent.offsetTop;
	...
	var bindGestureHandler = function($elem, vRect, startRender, stopRender, options) {
		...
		$elem.on("gesturestart", function(event) {
			...
			var offset = calcOffset(event.srcElement, elem);
			vRect.applyScale(ev.pageX, event.originalEvent.pageY, ev.scale, startRect);
	...
	$.fn.zoomAndScale = function(options) {
		...
		return this.each(function() {
			var $elem = $(this);
			var $parent = $('body');
			var transformOrigin = Modernizr.prefixed("transformOrigin").replace(/([A-Z])/g, function(str,m1){ return '-' + m1.toLowerCase(); }).replace(/^ms-/,'-ms-');
			var transform = Modernizr.prefixed("transform").replace(/([A-Z])/g, function(str,m1){ return '-' + m1.toLowerCase(); }).replace(/^ms-/,'-ms-');
			...
			virtualRect.applyScale(startRect.width / 2, startRect.height / 2, ( $parent.height() / $elem.height() ) * 0.9, startRect );
			$elem.css(transformOrigin, "0 0");
